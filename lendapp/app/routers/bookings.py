...
from app.dependencies import get_current_user, check_reminder_secret
...
def _send_new_booking_mail(booking: Booking, item: Item, db: Session):
    ...
    if owner and borrower:
        mail_new_booking(
            ...
            borrower_name=borrower.name, item_name=item.name,
...
def request_booking(data: BookingCreate, db: Session = Depends(get_db),
    ...
    borrower_id = None if is_external else current_user.id

    booking = Booking(
        item_id=data.item_id, borrower_id=borrower_id,
        external_name=data.external_name,
        date_from=data.date_from, date_to=data.date_to,
        note=data.note, status=status,
    )
    db.add(booking)
    if is_external:
...
def bookings_for_item(item_id: int, db: Session = Depends(get_db),
                      current_user: User = Depends(get_current_user)):
    # Nur Besitzer oder Ausleiher duerfen Buchungen sehen
    item = _get_item_or_404(db, item_id)
    bookings = db.query(Booking).filter(Booking.item_id == item_id).all()
    # Filtere sensible Daten fuer Nicht-Besitzer
    if item.owner_id != current_user.id:
        bookings = [b for b in bookings if b.borrower_id == current_user.id or b.status in [BookingStatus.approved, BookingStatus.external]]
...
def bookings_for_user(user_id: int, db: Session = Depends(get_db),
                      current_user: User = Depends(get_current_user)):
    # User darf nur eigene Buchungen sehen
...
def update_status(booking_id: int, data: BookingStatusUpdate,
    ...
    if borrower:
        if data.status == BookingStatus.approved:
            ...
            mail_booking_approved(
                borrower_email=borrower.email, borrower_name=borrower.name,
                item_name=item.name, owner_name=owner.name if owner else "",
                date_from=fmt_date(booking.date_from), date_to=date_to_str,
            )
...
def send_reminders(db: Session = Depends(get_db),
                   _: None = Depends(check_reminder_secret)):
    """Nur vom Cronjob-Container aufrufbar – geschuetzt mit REMINDER_SECRET."""
    ...
    for b in bookings:
        ...
        if borrower and item and owner:
            mail_return_reminder(
                borrower_email=borrower.email, borrower_name=borrower.name,
                item_name=item.name, owner_name=owner.name,
