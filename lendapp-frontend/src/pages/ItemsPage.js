...
function BookingModal({ item, userId, onClose, onBooked }) {
  ...
  useEffect(() => {
    api.getBlockedDates(item.id).then(setBlocked).catch(() => {});
  }, [item.id]);
  ...
  function handleCalendarSelect(date) {
    if (!fromDate || (fromDate && toDate)) {
      ...
      if (date <= fromDate) {
        ...
        // Check if any blocked date is in range
        const check = new Date(fromDate);
        let hasBlocked = false;
        while (check <= date) {
          if (blocked.some(r => {
            const ds = check.toISOString().slice(0, 10);
            if (r.open) return ds >= r.from;
            return ds >= r.from && ds <= r.to;
          })) { hasBlocked = true; break; }
          check.setDate(check.getDate() + 1);
        }
        if (hasBlocked) {
          setError("Der gewählte Zeitraum enthält gesperrte Tage.");
        } else {
          setToDate(date);
          setError("");
        }
  ...
  async function handleBook() {
    if (!fromDate || !toDate) return setError("Bitte Von- und Bis-Datum wählen");
    setLoading(true);
    try {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
        ...
        <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 12 }}>
          Klicke auf das Startdatum, dann auf das Enddatum.
        </p>
        <BookingCalendar
        ...
        {error && <div style={{ color: "var(--warn)", fontSize: 13, marginBottom: 12 }}>{error}</div>}
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-primary" style={{ flex: 1 }} onClick={handleBook} disabled={loading || !fromDate || !toDate}>
