...
function AddItemModal({ onClose, onSaved, groupId, userId }) {
  ...
  async function handleSave() {
    ...
    setLoading(true);
    try {
      await api.createItem({ ...form, max_days: Number(form.max_days) }, userId);
      onSaved();
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="form-group">
          ...
          <input className="form-input" placeholder="z.B. Bohrmaschine" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
        </div>
        <div className="form-group">
        ...
        <div className="form-group">
          ...
          <textarea className="form-input" placeholder="Zustand, Hinweise…" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} />
        </div>
        <div className="form-group">
          <label className="form-label">Max. Ausleihzeit (Tage)</label>
          <input className="form-input" type="number" min="1" max="365" value={form.max_days} onChange={e => setForm(f => ({ ...f, max_days: e.target.value }))} />
        </div>
...
export function ItemDetailPage() {
  ...
  return (
    <div>
        ...
        <div style={{ flex: 1, minWidth: 260 }}>
          <div className="card" style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
              <div className="item-emoji" style={{ fontSize: 40, width: 72, height: 72 }}>{CATEGORY_EMOJI[item.category] || "📦"}</div>
              <div style={{ flex: 1 }}>
            ...
            {bookings.map(b => (
              <div key={b.id} className="booking-row">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                  <span className={`badge ${b.status === "approved" ? "badge-green" : b.status === "pending" ? "badge-orange" : b.status === "returned" ? "badge-blue" : "badge-gray"}`}>{b.status}</span>
                  ...
                  {isOwner && b.status === "approved" && (
                    <button className="btn btn-sm badge-blue" style={{ padding: "3px 8px", fontSize: 11, background: "var(--blue-light)", color: "var(--blue)", border: "none" }} onClick={() => handleStatusChange(b.id, "returned")}>Zurück</button>
                  )}
                </div>
              </div>
            ))}
      ...
      {showBook && (
        <BookingModal item={item} userId={user.user_id} onClose={() => setShowBook(false)}
