...
export default function GroupPage() {
  const { user, groupId, groups, addGroup, setGroup, logout } = useAuth();
  ...
  return (
    <div>
      ...
      {success && <div style={{ background: "var(--accent-light)", color: "var(--accent)", padding: "10px 14px", borderRadius: "var(--radius-sm)", fontSize: 13, marginBottom: 16 }}>{success}</div>}
            ...
            {members.map(m => (
              <div key={m.user_id} className="booking-row">
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div className="avatar avatar-lg">{m.name && m.name[0] ? m.name[0].toUpperCase() : "?"}</div>
                  <div>
      ...
      {tab === "info" && !groupId && (
        <div className="card"><p style={{ fontSize: 14, color: "var(--text2)" }}>Du bist noch in keiner Gruppe.</p></div>
      )}

      {tab === "create" && (
        <div className="card">
          <div style={{ fontWeight: 600, marginBottom: 14 }}>Neue Gruppe erstellen</div>
          <div className="form-group"><label className="form-label">Gruppenname</label>
            <input className="form-input" placeholder="z.B. Familie Muller" value={newName} onChange={e => setNewName(e.target.value)} /></div>
          {error && <div style={{ color: "var(--warn)", fontSize: 13, marginBottom: 10 }}>{error}</div>}
          <button className="btn btn-primary btn-full" onClick={handleCreate} disabled={loading}>{loading ? "..." : "Gruppe erstellen"}</button>
        </div>
      )}
      ...
      {tab === "join" && (
        <div className="card">
          ...
          <div className="form-group"><label className="form-label">Gruppen-ID</label>
            <input className="form-input" placeholder="z.B. 1" type="number" value={joinId} onChange={e => setJoinId(e.target.value)} /></div>
          <div className="form-group"><label className="form-label">Einladungscode</label>
            <input className="form-input" placeholder="z.B. abc123" value={joinCode} onChange={e => setJoinCode(e.target.value)} /></div>
          {error && <div style={{ color: "var(--warn)", fontSize: 13, marginBottom: 10 }}>{error}</div>}
          <button className="btn btn-secondary btn-full" onClick={handleJoin} disabled={loading}>{loading ? "..." : "Beitreten"}</button>
        </div>
      )}
