...
export default function GroupPage() {
  const { user, groupId, setGroup } = useAuth();
  ...
  useEffect(() => {
    if (!groupId) return;
    api.getGroup(groupId).then(setGroupData);
    api.getMembers(groupId).then(setMembers);
  }, [groupId]);
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setGroup(g.id);
      setSuccess(`Gruppe „${g.name}" erstellt! Einladungscode: ${g.invite_code}`);
  ...
  function copyInvite() {
    if (group) {
      ...
      setSuccess("Einladungsinfo kopiert!");
      ...
      <div style={{ display: "grid", gap: 20, gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))" }}>
        <div className="card">
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="z.B. Familie Müller" value={newGroupName} onChange={e => setNewGroupName(e.target.value)} />
          </div>
        ...
        <div className="card">
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="z.B. 1" type="number" value={joinId} onChange={e => setJoinId(e.target.value)} />
          </div>
          <div className="form-group">
            <label className="form-label">Einladungscode</label>
            <input className="form-input" placeholder="z.B. abc123" value={joinCode} onChange={e => setJoinCode(e.target.value)} />
          </div>
          {error && <div style={{ color: "var(--warn)", fontSize: 13, marginBottom: 10 }}>{error}</div>}
  ...
  return (
    <div>
        ...
        <div className="card" style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 4, fontFamily: "var(--font-mono)" }}>
              ID: {group.id} · Code: <strong>{group.invite_code}</strong>
            </div>
          </div>
      ...
      <div className="card">
        ...
        {members.length === 0 && <div style={{ color: "var(--text3)", fontSize: 13 }}>Noch keine Mitglieder geladen.</div>}
      </div>
    </div>
  );
}
