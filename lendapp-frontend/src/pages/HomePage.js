    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: 400 }}>
        <p style={{ fontSize: 14, color: "var(--text2)", marginBottom: 16 }}>
          Du bist noch in keiner Gruppe. Geh zu <strong>Gruppe</strong> um eine zu erstellen oder beizutreten.
        </p>
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <div className="page-sub">Hallo {user?.name}!</div>
      </div>
      ...
      <div className="stats-row">
        ...
        <div className="stat-card">
          <div className="stat-val" style={{ color: pending.length > 0 ? "var(--warn)" : "var(--text)" }}>
            {pending.length}
          </div>
      ...
      {/* Offene Anfragen für meine Gegenstände */}
      {pending.length > 0 && (
        <div className="card" style={{ marginBottom: 24, borderLeft: "3px solid var(--warn)" }}>
          <div style={{ fontWeight: 600, marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}>
            <span>⏳</span> Jemand möchte etwas von dir ausleihen
          </div>
          {pending.map(b => (
            <div key={b.id} className="booking-row">
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 500, fontSize: 14 }}>
                  <strong>{b.borrower_name}</strong> möchte <strong>{b.item_name}</strong> ausleihen
                </div>
                <div style={{ fontSize: 12, color: "var(--text3)", marginTop: 2 }}>
                  {fmt(b.date_from)} – {fmt(b.date_to)}
                </div>
                {b.note && <div style={{ fontSize: 12, color: "var(--text2)", fontStyle: "italic", marginTop: 2 }}>„{b.note}"</div>}
              </div>
              <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                <button className="btn btn-sm" style={{ background: "var(--accent-light)", color: "var(--accent)", border: "none" }}
                  onClick={() => handleStatus(b.id, "approved")}>✓ Ja</button>
                <button className="btn btn-sm btn-danger"
                  onClick={() => handleStatus(b.id, "rejected")}>✕ Nein</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Ausgeliehene Gegenstände – wann zurück */}
      {lentOut.length > 0 && (
        <div className="card" style={{ marginBottom: 24, borderLeft: "3px solid var(--blue)" }}>
          <div style={{ fontWeight: 600, marginBottom: 14 }}>📤 Ausgeliehen von dir</div>
          {lentOut.map(item => {
            const activeBooking = myBookings.find(b => b.item_id === item.id && b.status === "approved");
            return (
              <div key={item.id} className="booking-row" style={{ cursor: "pointer" }}
                  ...
                  <div>
                    ...
                    {activeBooking && (
                      <div style={{ fontSize: 12, color: "var(--text3)" }}>
                        Bei: <strong>{activeBooking.borrower_name}</strong> · zurück am {fmt(activeBooking.date_to)}
                      </div>
                    )}
                  </div>
                </div>
          ...
          {myBookings.filter(b => b.status === "approved").map(b => (
            <div key={b.id} className="booking-row">
              <div>
                ...
                <div style={{ fontSize: 12, color: "var(--text3)" }}>
                  Von: <strong>{b.owner_name}</strong> · bis {fmt(b.date_to)}
                </div>
              </div>
      ...
      {loading ? <div style={{ color: "var(--text3)" }}>Lädt…</div> : (
        <div className="card-grid">
          {items.slice(0, 6).map(item => {
            const activeBooking = item.is_available ? null :
              myBookings.find(b => b.item_id === item.id && b.status === "approved");
            return (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  ...
                  {!item.is_available && activeBooking && (
                    <span style={{ fontSize: 11, color: "var(--text3)" }}>
                      frei ab {fmt(activeBooking.date_to)}
                    </span>
                  )}
                </div>
              </div>
